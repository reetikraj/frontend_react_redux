...
async function fetchInterview(interviewId) {
  try {
    const response = await fetch(
      `http://localhost:3005/api/v1/interviews/${interviewId}`
    )
...
function InterviewEdit() {
    ...
    async function get() {
      ...
      setInterview({
        interviews_users_attributes: {
          ...
          interview_date: init.interview_date,
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(`http://localhost:3005/api/v1/interviews/${interviewId}`, {
        method: "PATCH",
        body: formData,
      })

      console.log(response);
      window.location = "http://localhost:3004/#/interviews/"
    } catch (error) {
      console.log(error)
    }
  }
